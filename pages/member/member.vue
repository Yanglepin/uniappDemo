	...
	export default {
		...
		methods: {
			...
			toPage(list_i,li_i){
				...
				if(this.severList[list_i][li_i].name=='商品分类'){
					uni.navigateTo({
						url: '../left-category/left-category'
					})
				}
			}
		}
...
.header{
	...
	background-color:#6bbffa;width:92%;height:30vw;padding:0 4%;display:flex;align-items:center;
	.userinfo{
...
.orders{
	background-color:#6bbffa;width:92%;height:11vw;padding:0 4%;margin-bottom:calc(11vw + 40upx);display:flex;align-items:flex-start;border-radius:0 0 100% 100%;margin-top: -1upx;
	.box{
		...
		.label{
			...
			.icon{
				...
				.badge{position:absolute;width:4vw;height:4vw;background-color:#3da5ed;top:-1vw;right:-1vw;border-radius:100%;font-size:20upx;color:#fff;display:flex;align-items:center;justify-content:center;z-index: 10;}
